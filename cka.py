...
def get_cka(epath: str, inputs: npt.NDArray, a: arch.Architecture, use_initial: bool = True) -> Tuple[npt.NDArray, npt.NDArray]:
...
def plot_ckas(init_ckas: npt.NDArray, final_ckas: npt.NDArray, output: str):
    fig, (initial_ax, final_ax) = plt.subplots(ncols=2, sharey=True)
    fig.suptitle("CKA Score Over Pruning")

    def plot_cka(ax, cka: npt.NDArray, final: bool):
        mean_ckas = np.mean(cka, axis=0)
        std_ckas = np.std(cka, axis=0)
        ...
        ax.grid()
        ax.legend()
    plot_cka(initial_ax, init_ckas, False)
    plot_cka(final_ax, final_ckas, True)
    fig.tight_layout()
    fig.savefig(output)
...
plot_ckas(init_linear_ckas, final_linear_ckas, "cka_lenet_mnist.png")
