...
def run_seeded_init_experiment(
    hyperparameters: Hyperparameters,
    random_seed: int,
    create_model: Callable[[], keras.models.Model],
    dataset: ds.Dataset,
    ...
    pruning_rule: Callable,
    rewind_rule: Callable,
    seed_init: Callable[[keras.Model], None],
    init_callback: Callable[[keras.Model], Dict],
    global_pruning: bool = False,
    experiment_directory: str = './',
    log_level: int = logging.INFO,
):
    ...
    Args:
        ...
        sparsity_strategy (SparsityStrategy): Function which maps layer names
            to the appropriate level to sparsify them by.
        pruning_rule (Callable): Function used to prune model.
        rewind_rule (Callable): Function used for rewinding model weights.
        seed_init (Callable[[keras.Model], None]): Function to be called with the 
            Keras model as a parameter which performs the initialization at each set.
        init_callback (Callable[[keras.Model], Dict]: Callback to be called after each
            round of pruning on the trained weights which creates a dictionary of
            metrics on statistics related to the weights.
        global_pruning (bool, optional): Boolean flag for whether pruning is done globally
            or layerwise. Defaults to False.
        experiment_directory (str): Path to place all experimental data.
    """
    ...
    while True:
        ...
        seed_trial_data = history.SeededInitData(
            trial_data=trial_data,
            seeded_init_data=init_callback(model),
        )
        seed_trial_data.save_to(destination, C.TRIAL_DATAFILE)
