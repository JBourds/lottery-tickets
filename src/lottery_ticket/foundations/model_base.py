...
class ModelBase(object):
  ...
  def dense_layer(self,
    ...
    # Create the weights.
    weights = tf.Variable(
        initial_value=kernel_initializer(shape=[inputs.shape[1], units], dtype=tf.float32),
        trainable=True,
        name=name + '_w')
    ...
    if use_bias:
        bias = tf.Variable(
            initial_value=tf.zeros(shape=[units], dtype=tf.float32),
            trainable=True,
            name=name + '_b')
        output += bias
  ...
  def create_loss_and_accuracy(self, label_placeholder, output_logits):
      ...
      self._loss = loss_object(label_placeholder, tf.nn.softmax(output_logits))
