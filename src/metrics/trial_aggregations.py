...
def get_sparsity(trial: history.TrialData) -> float:
    ...
    """
    enabled_parameter_count: int = np.sum([np.sum(mask) for mask in trial.masks])
    total_parameter_count: int = np.sum([np.size(mask) for mask in trial.masks])
...
def get_early_stopping_iteration(trial: history.TrialData) -> int:
    ...
    """
    performance_evaluation_frequency: int = trial.train_accuracies.shape[0] // trial.test_accuracies.shape[0]
    nonzero_indices = np.nonzero(trial.train_accuracies == 0)[0]
    stop_index: int = len(trial.train_accuracies) if len(nonzero_indices) == 0 else nonzero_indices[0]
        ...
        for experiment in summary.experiments.values():
            ...
            for trial in experiment.trials.values():
                ...
                if pruning_step in trials_aggregated.keys():
                    trials_aggregated.get(pruning_step).append((trial_aggregation(trial)))
                else:
